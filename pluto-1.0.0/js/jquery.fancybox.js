...
(function (window, document, $, undefined) {
  ...
  const FancyBox = function (content, opts, index) {
    ...
    self.opts = mergeOpts({ index }, $.fancybox.defaults);
  ...
  $.extend(FancyBox.prototype, {
    ...
    addEvents() {
      ...
      self.$refs.container
        .on('click.fb-close', '[data-fancybox-close]', (e) => {
          e.stopPropagation();
          e.preventDefault();

          self.close(e);
        })
        .on('touchstart.fb-prev click.fb-prev', '[data-fancybox-prev]', (e) => {
          e.stopPropagation();
          e.preventDefault();

          self.previous();
        })
        .on('touchstart.fb-next click.fb-next', '[data-fancybox-next]', (e) => {
          e.stopPropagation();
          e.preventDefault();
    ...
    getThumbPos(slide) {
      const self = this;
      let rez = false;
      const { $thumb } = slide.opts;
      const thumbPos = $thumb && $thumb.length && $thumb[0].ownerDocument === document ? $thumb.offset() : 0;
      let slidePos;
  ...
  $.fn.fancybox = function (options) {
    ...
    if (selector) {
      ...
      $('body')
        ...
        .on('click.fb-start', selector, { options }, _run);
    } else {
  ...
  $(document).on('objectNeedsType.fb', (e, instance, item) => {
    ...
    $.each(media, function (providerName, providerOpts) {
      ...
      if (providerName === 'youtube') {
        url = url.replace(/&t=((\d+)m)?(\d+)s/, (match, p1, m, s) => `&start=${(m ? parseInt(m, 10) * 60 : 0) + parseInt(s, 10)}`);
...
(function (window, document, $) {
  ...
  Guestures.prototype.limitMovement = function () {
    ...
    const { canvasHeight } = self;

    const { distanceX } = self;
    const { distanceY } = self;

    const { contentStartPos } = self;
    ...
    let minTranslateX; let minTranslateY; let maxTranslateX; let maxTranslateY; let newOffsetX; let
      newOffsetY;
    ...
    if (distanceX > 0 && newOffsetX > minTranslateX) {
      newOffsetX = minTranslateX - 1 + (-minTranslateX + currentOffsetX + distanceX) ** 0.8 || 0;
    }

    //    <-
    if (distanceX < 0 && newOffsetX < maxTranslateX) {
      newOffsetX = maxTranslateX + 1 - (maxTranslateX - currentOffsetX - distanceX) ** 0.8 || 0;
    }

    //   \/
    if (distanceY > 0 && newOffsetY > minTranslateY) {
      newOffsetY = minTranslateY - 1 + (-minTranslateY + currentOffsetY + distanceY) ** 0.8 || 0;
    }

    //   /\
    if (distanceY < 0 && newOffsetY < maxTranslateY) {
      newOffsetY = maxTranslateY + 1 - (maxTranslateY - currentOffsetY - distanceY) ** 0.8 || 0;
    }
  ...
  Guestures.prototype.endZooming = function () {
    ...
    const { current } = self.instance;

    let newOffsetX; let newOffsetY; let newPos; let
      reset;

    const { newWidth } = self;
    const { newHeight } = self;
  ...
  Guestures.prototype.onTap = function (e) {
    ...
    const tapX = endPoints[0] ? endPoints[0].x - $(window).scrollLeft() - self.stagePos.left : 0;
    const tapY = endPoints[0] ? endPoints[0].y - $(window).scrollTop() - self.stagePos.top : 0;
...
(function (document, $) {
  ...
  const CLASS_LOAD = `${CLASS}-loading`;
...
(function (document, window, $) {
  ...
  function parseUrl() {
    ...
    const index = rez.length > 1 && /^\+?\d+$/.test(rez[rez.length - 1]) ? parseInt(rez.pop(-1), 10) || 1 : 1;
