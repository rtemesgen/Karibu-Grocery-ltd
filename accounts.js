...
(function () {
  ...
  function loadActivityLog() {
    try {
      activityLog = JSON.parse(localStorage.getItem(ACTIVITY_KEY)) || [];
    } catch (error) {
  ...
  function bindUI() {
    ...
    qs('loginBtn').addEventListener('click', () => {
      if (currentUser) {
        clearCurrentUser();
        addActivity('logout', { user: 'anonymous' });
        renderAll();
      } else {
        showLoginModal();
      }
    ...
    qs('loginFormLocal').addEventListener('submit', (e) => {
      e.preventDefault();
      handleLocalLogin();
    });
  ...
  function updateAccountTotals() {
    ...
    const expenses = transactions
      .filter((t) => t.type === 'expense' || t.type === 'procurement')
      .reduce((s, t) => s + (t.amount || 0), 0);
  ...
  function renderTransactions() {
    ...
    if (list.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; opacity:0.7;">No transactions found</td></tr>';
      return;
    }
  ...
  function showAddTransactionModal(editId) {
    ...
    if (editId) {
      ...
      if (tx) {
        qs('transactionType').value = tx.type;
        qs('transactionAmount').value = tx.amount;
        qs('transactionAccount').value = tx.account || 'cash';
        qs('transactionDescription').value = tx.description || '';
        qs('transactionDate').value = tx.date;
      }
    }
    qs('transactionModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
  }
  function hideTransactionModal() {
    qs('transactionModal').classList.add('hidden');
    ...
    if (editingTxId) {
      ...
      if (idx !== -1) {
        transactions[idx] = {
          ...
          description: desc,
          date,
          user: (currentUser && currentUser.username) || transactions[idx].user,
        };
        addActivity('update-transaction', { id: transactions[idx].id });
      }
    } else {
      const tx = {
      ...
      addActivity('add-transaction', { id: tx.id });
    }
    saveTransactions();
  ...
  function deleteTransaction(id) {
    ...
    saveTransactions();
    addActivity('delete-transaction', { id });
    renderTransactions();
    updateAccountTotals();
  }
  ...
  function handleLocalLogin() {
    ...
    saveCurrentUser();
    addActivity('login', { user: user.username });
    hideLoginModal();
    renderAll();
  }

  function updateProfileUI() {
    qs('profileRole').textContent = currentUser ? currentUser.role.toUpperCase() : 'Guest';
    qs('profileName').textContent = currentUser ? currentUser.username : 'Not logged in';
    qs('loginBtn').innerHTML = currentUser
  ...
  function addActivity(action, data) {
    const entry = {
      ...
      action,
      data: data || {},
      user: (currentUser && currentUser.username) || data?.user || 'System',
  ...
  function showActivityLog() {
    if (!currentUser || currentUser.role !== 'admin') {
      alert('Only admins can view activity logs');
      return;
    }
    qs('activityModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    renderActivityList();
  }
  function hideActivityLog() {
    qs('activityModal').classList.add('hidden');
    document.body.style.overflow = 'auto';
  }
  function renderActivityList() {
    const container = qs('activityList');
    if (!container) return;
    const search = (qs('activitySearch').value || '').toLowerCase();
    const from = qs('activityFrom').value;
    const to = qs('activityTo').value;
    ...
    container.innerHTML = list
      .map(
        (a) => `<div style="padding:0.5rem; border-bottom:1px solid #eee;"><div style="font-weight:600">${a.action}</div><div style="color:#6b7280; font-size:0.9rem">${a.user} • ${new Date(a.timestamp).toLocaleString()}</div><div style="margin-top:0.25rem; font-size:0.9rem">${JSON.stringify(a.data)}</div></div>`,
      )
      .join('');
  }
  function clearActivityLog() {
    if (!currentUser || currentUser.role !== 'admin') {
      alert('Only admins can clear activity log');
      return;
    }
    if (!confirm('Clear activity log?')) return;
    activityLog = [];
    saveActivityLog();
    addActivity('clear-activity-log', {});
    renderActivityList();
  }

  function exportActivityCsv() {
    const rows = [['ID', 'Action', 'User', 'Timestamp', 'Data']];
    activityLog.forEach((a) => rows.push([a.id, a.action, a.user, a.timestamp, JSON.stringify(a.data || {})]));
    const csv = rows
  ...
  function exportTransactionsCsv() {
    const rows = [['Date', 'Type', 'Amount', 'Account', 'Description', 'User']];
